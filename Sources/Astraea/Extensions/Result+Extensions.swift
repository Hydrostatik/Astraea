...
extension Astraea {
    ...
    public static func fmap<A,Failure,B>(_ a: Result<A, Failure>,
                                         _ f: @escaping (A) -> B) -> Result<B, Failure> { f <&> a }

    /// Prefix synonym of the `<&` operator. Replaces a wrapped value with another value.
    public static func rmap<A,Failure,B>(_ a: A,
                                         _ b: Result<B,Failure>) -> Result<A,Failure> { a <& b }
    ...
    public static func amap<A,Failure,B>(_ a: Result<(A) -> B, Failure>,
                                         _ b: Result<A, Failure>) -> Result<B, Failure> { a <*> b }

    /// Lifts wrapped values of type A, B out of their context, applies the transformation and returns a wrapped value.
    public static func liftA2 <A,Failure,B,C>(_ f: @escaping (A) -> ((B) -> C),
                                              _ a: Result<A, Failure>,
                                              _ b: Result<B,Failure>) -> Result<C,Failure> {
        f <&> a <*> b
    }

    /// Computes a and b, ignores the output of b and returns a
    public static func left<A,Failure,B>(_ a: Result<A,Failure>,
                                         _ b: Result<B,Failure>) -> Result<A,Failure> { a <* b }

    /// Computes a and b, ignores the output of a and return b
    public static func right<A,Failure,B>(_ a: Result<A,Failure>,
                                          _ b: Result<B, Failure>) -> Result<B, Failure> { a *> b }
}
